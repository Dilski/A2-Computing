	<?php
		...
		for ($i = $firstLastMonth; $i <= $lengthLastMonth; $i++)
			{
				...
				$sql = "SELECT * FROM `task` WHERE date(taskStart) <= '$actualDate' AND date(taskEnd) >= '$actualDate' AND `userId` = '$session_id'";
				...
				{	
					array_push($types, ' class="events prev-month"');
				}
		...
		for ($k = ($last+1); $k<42; $k++)
			{	
				...
				{	
					array_push($types, ' class="events next-month"');
				}
		...
		<tbody>
			<tr>
				<td<?php echo(array_shift($types));?>>
					...
					<?php tasks(array_shift($actualDates), $session_id); ?>
				</td>
				<td<?php echo(array_shift($types));?>>
					<h3 class="day"><span class="num"><?php echo(array_shift($numbers));?><span class="suffix">th</span></span></h3>
					<?php tasks(array_shift($actualDates), $session_id); ?>
